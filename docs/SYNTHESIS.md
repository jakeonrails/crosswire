...
| View layer | **Helpers + overridable partials.** Not ViewComponent, not Phlex. | Engine views are on the lookup path, so a consumer overrides by *creating a file*. ViewComponent globs from its own source dir and **cannot be shadowed** — its override story is "edit every call site." Also: zero added deps in a library pitched as framework-free. | 17 |
