...
module Crosswire
  # The real `crosswire:skills` generator specs, using Rails::Generators::TestCase,
  # live in skills_generator_test_runner.rb and run in a child process — for exactly
  ...
  class SkillsGeneratorTest < Minitest::Test
    RUNNER = File.expand_path("skills_generator_test_runner.rb", __dir__)

    def test_skills_generator_suite_passes_under_a_real_rails_boot
      output = IO.popen([RbConfig.ruby, RUNNER], err: [:child, :out], &:read)
      status = $?

      assert status.success?,
             "skills generator subprocess suite failed (exit #{status.exitstatus}):\n\n#{output}"
    end
  end
end
